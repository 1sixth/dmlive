...
impl Bilibili {
    ...
    async fn get_ws_info(&self, url: &str) -> anyhow::Result<(String, Bytes)> {
        ...
        let token = self
            .get_dm_token(&client, url, rid.to_string().as_str(), &cookie)
            .await?;
    ...
    async fn decode_msg(
        ...
        loop {
            ...
            if h.ver == 1 || h.ver == 0 {
                ...
                let mut dp = async_compression::tokio::bufread::DeflateDecoder::new(
                    BufReader::new(&data[16..h.packet_len as usize]),
                );
